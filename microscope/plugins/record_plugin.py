...
from qtpy.QtWidgets import (QAction, QGroupBox, QFormLayout, QHBoxLayout, 
                            QVBoxLayout, QLabel, QSpinBox, QLineEdit, QCheckBox)
...
class RecorderThread(QThread):
    ...
    def set_params(self, path='output.mp4', fourcc='avc1', fps=5, width=100, height=100):
        self.width = width
        self.height = height
    ...
    def _setup_recorder(self):
        self.video_recorder.open(str(self.path), self.fourcc, float(self.fps), (self.width, self.height))
...
class RecordPlugin(QObject):
    ...
    def qimage_to_mat(self, incomingImage: QImage):
        ...
        if self.timestamp:
            ...
            p.drawText(incomingImage.rect(), Qt.AlignHCenter, f'{datetime.now().strftime("%b-%d-%Y %H:%M:%S")}')
            p.end()
    ...
    def _record(self):
        ...
        if self.recording:
            ...
            self.current_filepath = Path(self.filename.parent)/Path(f'{self.filename.stem}_{self.start_time.strftime("%b-%d-%Y_%H%M%S")}.{self.file_extension}')
            ...
            self.video_recorder_thread.start(self.current_filepath, self.fourcc, self.fps, self.width, self.height)
        else:
            ...
            self.new_filepath = Path(f'{self.current_filepath.stem}_{self.end_time.strftime("%b-%d-%Y_%H%M%S")}.{self.file_extension}')
            self.current_filepath.rename(self.current_filepath.parent/self.new_filepath)
            print(f'Finished writing to {self.current_filepath.parent/self.new_filepath}')
            self._update_files()
    ...
    def _update_files(self):
        ...
        """
        files_found = list(self.filename.parent.glob(f'{self.filename.stem}*.{self.file_extension}'))
    ...
    def read_settings(self, settings: Dict[str, Any]):
        ...
        self.timestamp_color = settings.get('timestamp_color', QColor.fromRgb(0, 255, 0))
